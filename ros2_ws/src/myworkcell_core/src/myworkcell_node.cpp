...
#include <moveit/moveit_cpp/moveit_cpp.h>	//4.0
#include <moveit/moveit_cpp/planning_component.h>	//4.0
#include <geometry_msgs/msg/pose_stamped.hpp>	//4.0
...
//    this->declare_parameter("base_frame", "world"); //2.3 //4.0
  }
    // MoveIt setup //4.0
  void setup()
  {
...
//  void start()
  void start(const std::string& base_frame) //2.3
  {
...
//  }
    ...
    request->base_frame = base_frame; //2.3
    RCLCPP_INFO_STREAM(this->get_logger(), "Requesting pose in base frame: " << base_frame); //2.3
...
//    if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), future) != rclcpp::executor::FutureReturnCode::SUCCESS)
    ...
    geometry_msgs::msg::PoseStamped move_target; //4.0
    move_target.header.frame_id = base_frame; //4.0
    move_target.pose = response->pose; //4.0
...
private:
  ...
  moveit_cpp::MoveItCppPtr moveit_cpp_; //4.0
  moveit_cpp::PlanningComponentPtr planning_component_; //4.0
  moveit_cpp::PlanningComponent::PlanRequestParameters plan_parameters_; //4.0
};
...
int main(int argc, char **argv)
{
  ...
  std::string base_frame = app->get_parameter("base_frame").as_string(); //2.3
  //Wait for the vision node to receive data //2.3
  rclcpp::sleep_for(std::chrono::seconds(2)); //2.3
